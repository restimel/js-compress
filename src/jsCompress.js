...
/**
 * This lib is an helper for JS and JSON compression.
 * To send much smaller data through the web.
 *
 * It uses Hufman compression from a dictionary.
 * This dictionary can be generated and stored separtely.
 */

/** The default dictionary used for (de)compression */
const defaultJSONHist = new Map([
    ["{", 42],
    ["\"", 691],
    ["s", 190],
    ["a", 273],
 ...
 */
export function buildDictionary(histogram, options = {}) {
    const {
        addEscape = 0,
        verbose = false,
        debug = false,
 ...
 */
export function exportDictionary(dictionary, options = {}) {
    const {
        verbose = false,
        debug = false,
    } = options;
 ...
 */
export function extractDictionary(dictionary, options = {}) {
    const {
        verbose = false,
        debug = false,
    } = options;
    ...
    if (code && !decode) {
        ...
        Array.from(code, ([key, encoding]) => {
            let item = decode;
            for (const val of encoding.slice(0, -1)) {
                if (!item[val]) {
                    item[val] = {};
                }
                item = item[val];
            }
            item[encoding[encoding.length - 1]] = key;
        });
    }
    if (!code && decode) {
        code = new Map();
 ...
 */
export function pack(text, options = {}) {
    const {
